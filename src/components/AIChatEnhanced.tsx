...
import { ArrowUp, Sparkles, Square, ChevronDown, ChevronRight, Brain, Database, Globe, History, Plus, Trash2, BookOpen } from 'lucide-react';
...
interface Message {
  ...
  thinking?: string;
  isThinking?: boolean;
  thinkingPhase?: 'thinking' | 'synthesizing';
  timestamp?: number;
}
...
// Thinking indicator synced to backend stream phase
const ThinkingIndicator = ({ thinkingContent, phase = 'thinking' }: { thinkingContent?: string; phase?: 'thinking' | 'synthesizing' }) => {
  const [isOpen, setIsOpen] = useState(false);
  const { t } = useI18n();
  const phaseLabel = phase === 'synthesizing' ? t('Synthesizing') : t('Thinking');
    ...
    <div className="space-y-2">
      <Collapsible open={isOpen} onOpenChange={setIsOpen}>
        <CollapsibleTrigger className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
          <AnimatedBrainIcon />
          <span className="font-medium transition-all duration-300">{phaseLabel}...</span>
...
const AIChatEnhanced = ({ placeholderText }: AIChatEnhancedProps) => {
    ...
    if (abortControllerRef.current) {
      ...
      setMessages(prev => {
        ...
        if (lastMessage?.role === 'assistant') {
          ...
          lastMessage.thinkingPhase = undefined;
        }
        return newMessages;
      });
    }
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    setMessages(prev => [...prev, { role: 'assistant', content: '', thinking: '', isThinking: true, thinkingPhase: 'thinking' }]);

    abortControllerRef.current = new AbortController();
    let assistantContent = '';
    let thinkingContent = '';
    let isCurrentlyThinking = true;
    let currentPhase: 'thinking' | 'synthesizing' = 'thinking';
                ...
                {message.role === 'user' ? (
                  ...
                  <div className="text-sm text-foreground">
                    {message.thinkingPhase && (
                      <ThinkingIndicator thinkingContent={message.thinking} phase={message.thinkingPhase} />
                    )}
                    {!message.thinkingPhase && message.thinking && (
                      <ThinkingDisplay thinkingContent={message.thinking} />
                    )}
                      ...
                      <ReactMarkdown 
                        ...
                        components={{
                          p: ({ children }) => {
                            ...
                            if (researchPattern.test(text) || hasResearchInline) {
                              const cleanText = text.replace(/\[RESEARCH\]:?\s*/gi, '').trim();
                              return (
                                <p className="flex items-start gap-1.5 my-1">
                                  <span className="inline-flex items-center justify-center w-4 h-4 mt-0.5 flex-shrink-0">
                          ...
                          li: ({ children }) => {
                            ...
                            const hasResearch = /\[RESEARCH\]/i.test(text);
                            ...
                            if (hasResearch) {
                              ...
                              return (
                                <li className="flex items-start gap-1.5">
                                  <BookOpen className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400 mt-0.5 flex-shrink-0" />
                                  <span>{cleanText}</span>
                                </li>
                              );
                            }
