    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-muted/30 rounded-lg p-4 space-y-3 text-xs">
        ...
        <ol className="list-decimal list-inside space-y-2 text-muted-foreground">
          ...
          <li>
            {t('Open the pages you want to link with Biomedical, click')}{' '}
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        // Don't block user from proceeding even if update fails
        toast({
          title: t('Warning'),
          description: t('Setup saved locally. Please run database initialization script.'),
          variant: "default",
        });
      } else {
        toast({
      ...
      // Always proceed to trigger entry animation and navigation
      onComplete();
    } catch (error: any) {
      console.error('Onboarding error:', error);
      toast({
        title: t('Warning'),
        description: t('Proceeding to app. You may need to complete setup later.'),
        variant: "default",
      });
        ...
        <div className="mb-12 mt-20 text-left">
          <div className="flex items-center justify-between gap-4 mb-4">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-3xl tracking-tight font-normal" >Biomedical</h1>
            </div>
                    ...
                    {identityOptions.map((option) => {
                      ...
                      return (
                        <button
                          ...
                          className={`relative w-full flex items-center gap-4 p-4 rounded-xl border-2 transition-all duration-200 text-left group ${selectedIdentity === option.id
                            ? 'border-primary bg-primary/5 shadow-lg shadow-primary/10'
                            : 'border-border/50 hover:border-border hover:bg-muted/30'
                            }`}
                        >
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${selectedIdentity === option.id
                ...
                <div className="flex items-center gap-2">
                  ...
                  {step < totalSteps - 1 ? (
                    ...
                    <Button onClick={handleComplete} disabled={loading}>
                      {loading ? t('Setting up...') : t('Enter Biomedical')}
